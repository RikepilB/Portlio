## Auto snapshot — 2026-07-01T01:54:22.287Z (manual compaction)
- Branch: feat/landing-featured-projects
- Uncommitted (git status --short):
```
M .gitignore
 M CLAUDE.md
 M handoff.md
 M tsconfig.json
?? ,
?? ,+
?? .claude/agents/architect.md
?? .claude/agents/build-error-resolver.md
?? .claude/agents/chief-of-staff.md
?? .claude/agents/data-analyst.md
?? .claude/agents/database-reviewer.md
?? .claude/agents/doc-updater.md
?? .claude/agents/e2e-runner.md
?? .claude/agents/refactor-cleaner.md
?? .claude/agents/security-reviewer.md
?? .claude/agents/tdd-guide.md
?? .claude/commands/AVAILABLE.md
?? .claude/commands/redesign.md
?? .claude/hooks/
?? .claude/rules/common/
?? .claude/rules/typescript/
?? .claude/settings.local.json
?? .claude/skills/AGENTS.md
?? .claude/skills/README.md
?? .claude/skills/backend-patterns/
?? .claude/skills/baseline-ui/
?? .claude/skills/ckm-design-system/
?? .claude/skills/ckm-ui-styling/
?? .claude/skills/coding-standards/
?? .claude/skills/continuous-learning/
?? .claude/skills/deployment-patterns/
?? .claude/skills/frontend-patterns/
?? .claude/skills/impeccable/
?? .claude/skills/python-patterns/
?? .claude/skills/security-review/
?? .claude/skills/security-scan/
?? .claude/skills/shadcn/
?? .claude/skills/subagent-driven-development/handoff.md
?? .claude/skills/tdd-workflow/
?? .claude/skills/ui-ux-pro-max/
?? .claude/skills/verification-loop/
?? .github/
?? .mcp.json
?? .playwright-mcp/
?? ADD-PROJECT-EL-UMBRAL.md
?? CLAUDE.local.md
?? audit-home-1280.jpeg
?? audit-home-1280b.jpeg
?? bottom
?? docs/decisions.md
?? docs/handoff/
?? docs/plans/
?? e.type
?? legacy
?? opencode.json
?? tests/
?? verify-home-375-new.jpeg
?? verify-home-375.jpeg
?? verify-projects-1280.jpeg
?? verify-projects-375-new.jpeg
?? verify-projects-375.jpeg
?? verify-skills-1280.jpeg
```
- Diff stat:
```
.gitignore    |  3 +++
 CLAUDE.md     | 32 +++++++++++++++++++++++---------
 handoff.md    | 31 +++++++++++++++++--------------
 tsconfig.json |  3 ++-
 4 files changed, 45 insertions(+), 24 deletions(-)
```
