...
**Featured-project refresh shipped to `main` through PR #35 at `2584250` and is live on `richardpillaca.com`; focused hydration follow-up PR #36 is open.** The six Home projects use current product screenshots and source-backed EN/ES content, stacks, metrics, demo URLs, GitHub URLs, and Foglamp codebase-map URLs. Home cards restore stack pills plus direct Details, Demo, Code, and Codebase map controls; images and titles link to localized case studies. Detail galleries use horizontal `object-contain` framing. GitHub CI, Vercel preview, and CodeRabbit passed, including fixes for all three valid review findings. Post-deploy browser verification confirmed the new content and all six images, then caught React hydration error 418 caused by mounting Vercel Analytics through a Vercel-only conditional. PR #36 replaces it with the documented stable production-mode mount; the local production build hydrates without a React error and the full lint/type/test/build gate passes. Existing unrelated untracked scaffolding remains excluded.
