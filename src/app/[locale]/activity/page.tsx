...
export default async function ActivityPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <div className="shell min-h-screen pb-24 pt-32">
    ...
    <p className="mb-8 max-w-2xl text-sm leading-7 text-muted"><a href="https://github.com/RikepilB" target="_blank" rel="noopener noreferrer" className="text-gold-bright hover:underline">GitHub ↗</a>{locale === 'es' ? ' · 2.268 en el último año · sep. 2026' : ' · 2,268 in the past year · Sep 2026'}{' · '}<a href="https://www.linkedin.com/in/richard-pillaca" target="_blank" rel="noopener noreferrer" className="text-gold-bright hover:underline">LinkedIn ↗</a>{locale === 'es' ? ' · publicaciones completas' : ' · full posts'}</p>
