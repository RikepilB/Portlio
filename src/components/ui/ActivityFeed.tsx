      ...
      <div className="divide-y divide-rule" aria-live="polite">
        {!preview && (filter === 'all' || filter === 'project') && <section className="py-7" aria-labelledby="my-projects">
          ...
          <div className="mt-6 grid gap-x-10 sm:grid-cols-2">
            {getProjects(locale).filter((project) => activityProjectSlugs.includes(project.slug)).map((project) => <article key={project.slug} className="border-t border-rule py-5">
            ...
            {additionalActivityProjects.map((project) => <article key={project.href} className="border-t border-rule py-5"><h3 className="font-display text-2xl text-matte"><a href={project.href} target="_blank" rel="noopener noreferrer" className="hover:text-gold-bright">{project.title} ↗</a></h3><p className="mt-2 text-sm leading-7 text-ink-on-felt">{project.description[locale]}</p></article>)}
          </div>
