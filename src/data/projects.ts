...
export const projects: Project[] = [
    ...
    },
    {
        ...
        readTime: '9 min read',
        overview:
            'ExamVault is a full-stack platform that takes instructors from blank page to graded insight — generate randomized multiple-choice exams in seconds, auto-grade entire sessions, and see exactly how a class performed through a live analytics dashboard, all in one place. No more spreadsheets, no more manual marking, no more waiting days for results. Built over four months for a real client, the system replaces a slow, error-prone workflow with one fast, defensible pipeline that any instructor can run without a data background. I designed and built the backend engine for randomized exam generation, created the analytics dashboard that turns raw answer sheets into question-level difficulty trends and performance breakdowns, and served as Scrum Master — owning sprint planning, unblocking the team, and keeping a two-developer build shipping on a tight semester deadline. The result is a product that is genuinely usable today and a project that stretched me across architecture, data visualization, debugging under pressure, and direct stakeholder communication.',
        problem:
